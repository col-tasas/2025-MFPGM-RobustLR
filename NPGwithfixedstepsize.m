...
eta_BS_4=[]; %step size for noise 10^-4
eta_BS_2=[0.05*ones(1,30)]; %step size for noise 10^-2
eta_BS_0=[]; %step size for noise 10^-0
eta_BS=[eta_BS_4,eta_BS_2,eta_BS_0];
AmpNoise_forloop_BS=[0.0001*ones(1,numel(eta_BS_4)),0.1*ones(1,numel(eta_BS_2)),1*ones(1,numel(eta_BS_0))];
...
[Pstar1,K_initial]=idare(A,B,100*Q,R);
K_initial=-K_initial;
episode=110;
...
for counter=1:num_BS
    ...
    P0=dlyap((A+B*kcurrentNBS)',Q+kcurrentNBS'*R*kcurrentNBS);
    for i=1:episode
        ...
        kcurrentNBS=kcurrentNBS-eta_BS(counter)*1/norm(R+B'*P0*B)*nablaK*inv(SigmaK);
        catch
            break;
        end
    end
...
BS1=zeros(episode,1);
for i=1:episode
    BS1(i,1)=(ckBS(i,1)-cKstar*10)/(cKstar*10);
end
figure;
...
function X=randommatrix(K,r)
    ...
    [a,b]=size(K);
      lower = -10000;
      upper = 10000;
    random = lower + (upper-lower).*rand(a*b,1);
    target_norm=r;
    X=reshape(random,[a,b]);
